...
Функционал: Экспортный сценарий создания ордера и акта на списание

Как Тестировщик я хочу
Создать ордер и акт на списание
чтобы проверить работает ли эта функция   

Контекст:
	Дано Я запускаю сценарий открытия TestClient или подключаю уже существующий

Сценарий: Я создаю ордер и акт на списание на Ленина
*Создание ордера 
	И В командном интерфейсе я выбираю 'Склад и доставка' 'Ордер на отражение недостач товаров'
	...
	И из выпадающего списка с именем 'FieldSelector' я выбираю точное значение "Подразделение"
	И из выпадающего списка с именем 'Pattern' я выбираю по строке "Бердск - Ленина, 89  (Новосибирская область)"
...
*Создание Акта
				//	//придумать, как выбрать необходимый ордер. сейчас выбирается крайний (запомнить номер документа) (в название сценария вывести номер)
...
//	Тогда открылось окно 'Ордера на отражение недостач товаров'
	Когда открылось окно "Ордер на отражение недостач товаров ФФРЧ-*"
