	...
	*Создание приходного ордера
		...
		И я нажимаю на кнопку с именем 'ФормаЗаписать'
			...
			*Провекрка распроведения
				Когда открылось окно "Приходный ордер на товары ФФРЧ-* от *"
...
Контекст:
	И я подключаю TestClient "Администратор Л" логин "Администратор" пароль ""
	И я закрываю все окна клиентского приложения
			И Я создаю Расценку товаров для артикула "3172430" с ценой "49" и артикула "3174665" с ценой "121" на Ленина
					//	3172430 - пастилки 
					//  3174665 - корвалол
//Сценарий: Создание Корректировки на недостачу (и возврат)
Контекст:
	И я подключаю TestClient "Заведующий аптекой Л" логин "Заведующий аптекой" пароль ""
	...
	*Создание Расходного ордера 
		...
		И я нажимаю на кнопку с именем 'ФормаПровести'
		И Пауза 1
																									//Не заполняется распоряжение. Провести невозможно
...
//	Сценарий: Перемещение товара в Карантин
	*Создание ордера на перемещение
		И Я создаю перемещение товара из "Торговый зал" В "Карантин" на Ленина
	//
//	Сценарий: Перемещение товара из Карантина
	*Создание ордера на перемещение
		И Я создаю перемещение товара из "Карантин" В "Торговый зал" на Ленина

	*Списание и оприходование		

		*Сценарий: Создание ордера и акта на списание вручную
		И Я создаю ордер и акт на списание на Ленина

		*Сценарий: Создание ордера и акта на оприходование вручную
	  	И Я создаю ордер и акт на оприходование на Ленина

//
//	Сценарий: Межаптечная передача в офис
	//	И я создаю перемещение товара в офис на Ленина